...
WHISPER_BIN = _resolve("WHISPER_BIN", "whisper-cli", os.path.expanduser("~/Desktop/Whisper/whisper.cpp/build/bin/whisper-cli"))
_def_whisper_model = os.path.expanduser("~/Desktop/Whisper/whisper.cpp/models/ggml-large-v3.bin")
WHISPER_MODEL = os.environ.get("WHISPER_MODEL") or (
    _def_whisper_model if os.path.exists(_def_whisper_model)
    else os.path.join(HERE, "../models/ggml-large-v3.bin")
)
_def_yunet = os.path.expanduser("~/Documents/HaolamShorts/2026-08-30_11-44-30/work/yunet.onnx")
YUNET_MODEL = os.environ.get("YUNET_MODEL") or (
    _def_yunet if os.path.exists(_def_yunet)
    else os.path.join(HERE, "../models/yunet.onnx")
)
...
def stage_a(src, plan, workdir, plan_path=None):
    ...
    if "preserve" in plan and plan["preserve"]:
        pres_str = ";".join(f"{s}-{e}" for s, e in plan["preserve"])
